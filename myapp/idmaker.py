...
class IdMaker:
    ...
    def pak_id_card(idcard):
        font = IdMaker.font(size=30,type="Bold")
        template = Image.open("myapp/static/template/pak/pak_template.png")
        img = Image.open(idcard.image)
        passport_photo = img.resize((168, 217), Image.ANTIALIAS)
        passport_photo_sm = img.resize((44, 53), Image.ANTIALIAS)
        template.paste(passport_photo, (450, 89, 618, 306))
        template.paste(passport_photo_sm, (66, 314, 110, 367))
        draw = ImageDraw.Draw(template)
        draw.text((187, 115), idcard.name, font=font,fill=(63,70,66))
        draw.text((187, 187), idcard.father_name, font=font, fill=(63,70,66))
        draw.text((187, 233), idcard.gender, font=font, fill=(63,70,66))
        template.resize((450, 300))
        return template
    ...
    def balaji_id_card(idcard):
        ...
        draw = ImageDraw.Draw(template)
        photo = Image.open(idcard.image).resize((547, 707), Image.ANTIALIAS)
